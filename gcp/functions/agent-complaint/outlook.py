...
class OutlookService:
    ...
    def create_draft(self, mailbox_email, subject, content, recipient_email=None):
        if not self.token:
            self.authenticate()
        ...
        message = {
            ...
            "toRecipients": [
                {
                    "emailAddress": {
                        "address": recipient_email or mailbox_email
                    }
                }
            ]
        }
        ...
        try:
            ...
            print(f"Failed to create draft for {mailbox_email}: {e}")
            raise
